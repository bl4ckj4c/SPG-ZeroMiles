...
credBackup = credentials.Certificate(
...
appBackup = firebase_admin.initialize_app(credBackup, {
...
dbBackup = firestore.client(appBackup)
...
def copyDataFromBackupTo2And3PlusLocal():
    ...
    # User
    docs = dbBackup.collection(u'User').get()
    for doc in docs:
    ...
    # Farmer
    docs = dbBackup.collection(u'Farmer').get()
    for doc in docs:
    ...
    # Product
    docs = dbBackup.collection(u'Product').get()
    for doc in docs:
    ...
    # Order
    docs = dbBackup.collection(u'Order').get()
    for doc in docs:
    ...
    # Product by Farmers
    docs = dbBackup.collection(u'Product by Farmers').get()
    for doc in docs:
...
if __name__ == '__main__':
    ...
    makeLocalCopyOfFirebase(sourceDB=3)
